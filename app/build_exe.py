...
================================
만든이: cmlee@kaeri.re.kr (문의·개선 요청 환영)
...
실행: 업무스케줄.exe 더블클릭
  ...
  - 창을 닫으면 앱이 1분 안에 완전히 종료됩니다 (백그라운드에 남지 않음).
...
주의: 개발용(파이썬 스크립트판) 앱을 같은 PC에서 함께 쓰는 경우,
둘을 동시에 실행하면 서로 상대 서버를 교체하며 각자 다른 데이터 폴더를
바라봅니다. 한 PC에서는 한 가지 방식만 사용하세요.
"""
