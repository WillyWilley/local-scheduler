...
- `day_notes[]` (선택) — 달력의 특정 날짜에 붙는 메모. 최상위 키이며
  ...
  **그 항목의 그 날짜 칸** 메모(격자 칸 하나에 📝 메모지 표시), 없으면 날짜 전체
  메모(눈금 📝 표식). 날짜·항목 조합당 하나. 격자 칸·막대 클릭 = 항목 메모,
  눈금 클릭 = 날짜 전체 메모, 섹션·지난 일정 행 클릭은 무시. 막대 클릭은 좌표로
  날짜를 계산하고, 드래그 직후 클릭·더블클릭(편집창)과 충돌하지 않게 지연 오픈.
  hover 시 내용 툴팁. 항목이 삭제되면 그 항목의 메모도 저장 시 함께 정리된다.
  빈 메모는 삭제로 취급하며,
