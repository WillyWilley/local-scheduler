...
- `day_notes[]` (선택) — 달력의 특정 날짜에 붙는 메모. 최상위 키이며
  `{"date": "YYYY-MM-DD", "text": "...", "item_id": 123}` 형태. `item_id`가 있으면
  **그 항목의 그 날짜 칸** 메모(격자 칸 하나에 📝 메모지 표시), 없으면 날짜 전체
  메모(눈금 📝 표식). 날짜·항목 조합당 하나. 에디터에서 격자 빈칸 hover 시 + 버튼,
  클릭으로 추가·수정 (눈금 클릭 = 날짜 전체 메모). 빈 메모는 삭제로 취급하며,
  하나도 남지 않으면 키 자체를 지운다
...
+ **편집 UI 변경 시: 화면 동작만으로 완료 선언 금지.** 실제 편집 → 저장 →
  `data/schedule_data.json`에 값이 기록됐는지까지 확인한다.
  (dhtmlx v10은 커스텀 폼 블록의 `map_to:"auto"` 반환값을 무시한다 —
   외부 라이브러리 규약은 기억이 아니라 `assets/`의 번들 소스로 확인할 것)
