...
- `day_notes[]` (선택) — 항목이 아니라 **달력의 특정 날짜**에 붙는 메모.
  최상위 키이며 `{"date": "YYYY-MM-DD", "text": "..."}` 형태, 날짜당 하나.
  일간 화면에서 눈금 📝 표식 + 격자 열 틴트로 표시하고 내용은 클릭해야 보인다
  (에디터는 격자 빈칸 hover 시 + 버튼, 클릭으로 추가·수정). 빈 메모는 삭제로 취급하며,
